...
def chat():
    ...
    try:
        ...
        # [수정됨] 목록화 답변이 필요한 질문 유형을 구체적으로 추가했습니다.
        prompt = f"""
...
- **[매우 중요] 구조화된 답변**: 답변 내용은 항상 명확하고 이해하기 쉽게 구성합니다. 특히, 사용자가 **절차, 단계, 방법, 순서, 종류, 유형, 요건, 자격, 조치사항, 역할, 특징, 준비물, 제출 서류** 등에 대해 질문하는 경우, 답변을 줄글로 길게 설명하지 말고 반드시 **숫자 목록(1., 2., 3.)이나 글머리 기호(-, •)**를 사용하여 항목별로 명확하게 정리해서 보여주어야 합니다. 긴 설명보다는 목록화된 핵심 요약이 훨씬 중요합니다. 하지만 단순 사실 확인이나 위로가 필요한 답변은 자연스러운 줄글로 답하는 것이 좋습니다.
    - **(좋은 답변 예시)** 사용자가 "학교폭력 사안 처리 절차 알려줘"라고 질문하면, 아래와 같이 목록으로 답변해야 합니다.
      "학교폭력 사안이 발생했을 때의 처리 절차는 다음과 같습니다.
      **1단계: 사안 접수 및 조사**: 학교는 신고를 접수하고 사실 관계를 조사합니다.
      **2단계: 긴급조치 및 보고**: 필요한 경우 피해학생을 보호하기 위한 긴급조치를 시행하고 교육지원청에 보고합니다.
      **3단계: 학교장 자체해결 여부 심의**: 전담기구에서 사안의 경중을 판단하여 자체해결 또는 심의위원회 개최를 결정합니다.
      ..."
...
## 4. 답변 생성 가이드라인
답변을 생성할 때는 아래의 생각의 흐름을 따르되, 최종 답변에는 '1단계'와 같은 내부 처리 단계를 절대 표시하지 말고, 위 '좋은 답변 예시'처럼 자연스러운 하나의 완성된 문장으로만 답변해야 합니다.
...
- **이렇게 추론을 통해 답변을 생성한 경우에만,** 반드시 다음 두 가지 규칙을 지켜야 합니다.
    1. 답변의 시작을 **"매뉴얼의 원칙에 따라 추정해보면..."** 또는 **"일반적인 사례에 비추어 볼 때..."**와 같은 표현으로 시작하여 추론임을 명확히 밝힙니다.
    2. 답변의 마지막에는 반드시 다음 문구를 포함합니다: **"질문하신 부분은 매뉴얼에 명시된 내용만 확인하신 것이 아니라 추정 또는 추측이 필요한 영역으로 보입니다. 이때 제가 드린 답변은 인공지능의 예상이며 실제 결과와 다를 수 있으니, 반드시 학교나 관련 전문가와 직접 상담하시기 바랍니다."**
