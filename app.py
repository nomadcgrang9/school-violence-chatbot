...
def chat():
    ...
    try:
        ...
        # [수정됨] 면책 조항이 붙는 조건을 더욱 명확하게 수정했습니다.
        prompt = f"""
...
**[중요] 이 단계에서 생성된 답변에는 절대 "이것은 AI의 예상이며..."와 같은 면책 조항을 포함해서는 안 됩니다. 사실만을 간결하게 전달하세요.**

### 3단계: 추론 확장 (Logical Inference)
만약 질문에 대한 답이 <매뉴얼>에 명시적으로 없다면, 질문과 가장 관련성이 높은 원칙, 판단 기준, 관련 규정들을 종합하여 가장 가능성 있는 결과를 논리적으로 추론합니다.

### 4단계: 안전장치 및 권고 (Safety Net & Recommendation) - 오직 3단계 답변에만 적용
**오직 3단계의 추론을 통해 답변을 생성한 경우에만**, 반드시 다음 두 가지 요소를 포함해야 합니다.
- **추론 명시**: 답변이 추론에 의한 것임을 명확히 밝히기 위해, 문장의 시작을 "매뉴얼의 원칙에 따라 추정해보면..." 또는 "일반적인 사례에 비추어 볼 때..." 와 같은 표현으로 시작해야 합니다.
- **면책 조항 및 전문가 상담 권고**: 답변의 마지막에는 반드시 다음의 문구를 포함해야 합니다. "추정 또는 추론과 관련된 질문하신 경우 제가 하는 답변은 인공지능의 예상이며 실제 결과와 다를 수 있으니, 반드시 학교나 관련 전문가와 직접 상담하시기 바랍니다."
